...
# Specific file to analyze (single-file mode)
...
def _write_csv_header(path, fieldnames):
...
if __name__ == "__main__":
    # Batch mode over many mu values in MATRIX_DIR: write CSV gamma(mu, m)
    if MATRIX_DIR and os.path.isdir(MATRIX_DIR):
        pkl_files = _find_pkls_in_dir(MATRIX_DIR)
        if not pkl_files:
            ...
            for path in pkl_files:
                ...
                for m in ms:
                    ...
                    row[f"m{m}"] = float(val) if (val is not None and np.isfinite(val)) else ""

                _append_csv_row(csv_path, fieldnames, row)

            print(f"\nDone. CSV saved to: {csv_path}", flush=True)
    else:
        # Original single-file behavior (with plots and diagnostics)
        print("=== Loading matrix and computing eigenvectors ===", flush=True)

        if not MATRIX_FILE:
            raise RuntimeError("MATRIX_FILE is empty and MATRIX_DIR is not a directory. "
                               "Set MATRIX_FILE to a specific .pkl or MATRIX_DIR to a folder.")
