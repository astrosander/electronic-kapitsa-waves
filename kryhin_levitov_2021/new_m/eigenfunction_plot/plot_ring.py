...
import os
print("[IMPORT] os imported", flush=True)
import argparse
print("[IMPORT] argparse imported", flush=True)
import numpy as np
print("[IMPORT] numpy imported", flush=True)
import pickle
print("[IMPORT] pickle imported", flush=True)
from scipy.sparse import csr_matrix, diags
print("[IMPORT] scipy.sparse imported", flush=True)
from scipy.sparse.linalg import eigsh, eigs
print("[IMPORT] scipy.sparse.linalg imported", flush=True)

# Use non-interactive backend for headless compute nodes (prevents blocking)
# MUST set backend BEFORE importing pyplot
print("[IMPORT] Setting matplotlib backend...", flush=True)
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend for headless nodes
print("[IMPORT] matplotlib backend set to Agg", flush=True)
from matplotlib import pyplot as plt
print("[IMPORT] matplotlib.pyplot imported", flush=True)

# Numba acceleration
print("[IMPORT] Checking for numba...", flush=True)
USE_NUMBA = False
try:
    import numba
    from numba import njit
    USE_NUMBA = True
    print("[IMPORT] numba imported successfully", flush=True)
except ImportError as e:
    print(f"[IMPORT] Numba import failed: {e}", flush=True)

if USE_NUMBA:
    # Optional diagnostics
    print("[IMPORT] Running numba diagnostics...", flush=True)
    try:
        print("cpu_count:", os.cpu_count(), flush=True)
        print("numba threads:", numba.get_num_threads(), flush=True)
        print("numba layer:", numba.threading_layer(), flush=True)
    except Exception as e:
        print(f"[IMPORT] Numba OK; diagnostics failed: {repr(e)}", flush=True)

print("[IMPORT] All imports completed", flush=True)
...
# Raw eigenvector computation settings (can be slow for large matrices)
# WARNING: Raw eigenvector computation can take several minutes for large matrices
...
N_RAW_EIGS = 6                     # Number of raw eigenvectors to compute (reduced for speed)
N_RAW_PLOT = 4                     # Number of raw eigenvectors to plot (reduced for speed)
...
# Module-level print to confirm script is loading
print("[MODULE] Script module loaded, imports completed", flush=True)
...
def compute_eigenfunctions_by_mode(Ma, meta, ms=[0, 1, 2, 3, 4, 5, 6, 7, 8]):
    ...
    print(f"  [compute_eigenfunctions_by_mode] Starting for modes {ms}...", flush=True)
    ...
    print(f"  [compute_eigenfunctions_by_mode] Reconstructing coordinates...", flush=True)
    px, py = reconstruct_px_py(meta)
...
if __name__ == "__main__":
    print("[START] Script execution started", flush=True)
    
    # Parse command-line arguments
    print("[START] Parsing command-line arguments...", flush=True)
    ...
    print(f"[START] Matrix file: {MATRIX_FILE}", flush=True)

    print("=== Loading matrix and computing eigenvectors ===", flush=True)
    
    # Load the specific matrix file
    print(f"[LOAD] Loading matrix from: {os.path.basename(MATRIX_FILE)}", flush=True)
    print(f"[LOAD] Opening file...", flush=True)
    try:
        with open(MATRIX_FILE, 'rb') as fp:
            print(f"[LOAD] Reading pickle file (this may take a moment for large files)...", flush=True)
            Ma, meta = pickle.load(fp)
            print(f"[LOAD] Pickle file loaded successfully", flush=True)
    except Exception as e:
        print(f"[ERROR] Failed to load pickle file: {e}", flush=True)
        raise
    
    print(f"[LOAD] Extracting metadata...", flush=True)
    Theta = float(meta.get("Theta", 0.0))
    ...
    print(f"[COMPUTE] Starting compute_eigenfunctions_by_mode for modes {ms}...", flush=True)
    eigenfunctions, eigenvalues, px, py = compute_eigenfunctions_by_mode(Ma, meta, ms=ms)
    print(f"[COMPUTE] compute_eigenfunctions_by_mode completed", flush=True)
