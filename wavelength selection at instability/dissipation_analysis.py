...
def plot_W_vs_u_d(u_d_values, W_values, un_values=None, meta_ref=None, n_ref=None, output_file="W_vs_u_d", also_plot_onset_test=True):
    ...
    if meta_ref is not None:
        u_c = 0.379#predicted_uc(meta_ref, n_ref=n_ref)
        ...
        mask_fit = (u >= 0.39) & (u <= 0.41) & np.isfinite(W_over_W0)
        if np.sum(mask_fit) > 1:
            u_fit = u[mask_fit]
            W_fit = W_over_W0[mask_fit]
            coeff_fit = np.polyfit(u_fit, W_fit, 1)
            u_fit_range = np.linspace(u.min(), u.max(), 200)
            W_fit_line = np.polyval(coeff_fit, u_fit_range)
            ax1.plot(u_fit_range, W_fit_line, linestyle="--", color="red", linewidth=1.5, alpha=0.7,
                    label=r"fit ($0.45 \leq u_d \leq 0.5$)")
            print(f"[W/W0 fit] u_d=[0.45, 0.5]: slope={coeff_fit[0]:.4g}, intercept={coeff_fit[1]:.4g}")
...
if __name__ == "__main__":
    ...
    base_dir = r"D:\Рабочая папка\GitHub\electronic-kapitsa-waves\dn vs u_d\multiple_u_d\w=0.14_modes_3_5_7_L10(lambda=0.0, sigma=-1.0, seed_amp_n=0.03, seed_amp_p=0.03)_Dn=0p01_Dp=0p10"#w=1_modes_3_5_7_L10(lambda=0.0, sigma=-1.0, seed_amp_n=0.001, seed_amp_p=0.001)"
